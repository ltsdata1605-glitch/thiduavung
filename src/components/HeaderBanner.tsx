...
export const HeaderBanner: React.FC<HeaderBannerProps> = ({
  ...
  const toggleChannel = (channel: Channel) => {
    ...
    // Nếu bộ lọc kênh chỉ chọn "TGD": Nhóm N.Hàng sẽ mặc định bỏ chọn nhóm N.Hàng CE & GD
    const isOnlyTgd = nextChannels.length === 1 && nextChannels[0] === 'TGD';
    const wasOnlyTgd = selectedChannels.length === 1 && selectedChannels[0] === 'TGD';
    ...
    const defaultNonCeString = nonCeGroups.join(',');

    if (isOnlyTgd) {
      if (nonCeGroups.length > 0 && nonCeGroups.length < categoryGroupList.length) {
        setSelectedCategoryGroup(defaultNonCeString);
      }
    } else if (wasOnlyTgd && !isOnlyTgd) {
      // Khôi phục về "Tất cả" nếu trước đó đang dùng danh sách mặc định bỏ CE&GD
      if (selectedCategoryGroup === defaultNonCeString) {
        setSelectedCategoryGroup('ALL');
      }
    }
  };
    ...
    <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm relative space-y-3 transition-all">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pl-2">
        ...
        <div className="flex items-center gap-3 min-w-0">
          {/* Scope Selector: TỔNG / VÙNG / SIÊU THỊ / NHÓM */}
